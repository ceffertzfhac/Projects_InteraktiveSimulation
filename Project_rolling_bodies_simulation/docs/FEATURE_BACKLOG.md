...
~~1.  **Erweiterte Spuren-Logik:**
    ...
    - Die Spur muss unabhängig an- und abwählbar sein.~~ ✓ (bereits implementiert)
...
~~2.  **Bezugssystem-Einfluss auf Diagramme:**
    ...
    - Betrifft insbesondere die Positionen ($x, y$), aber auch die Geschwindigkeits- und Beschleunigungskomponenten ($v_x, v_y, a_x, a_y$).~~ ✓ (in 1.8.0 implementiert)

~~3.  **Beta-Steuerung & K-Faktor Validierung:**
    - Beta-Slider für alle Körper verfügbar machen, falls ein Hohlkörper aktiv ist.
    - Konsolidierung der K-Faktor-Berechnung.~~ ✓ (in 1.8.9/1.9.0 implementiert)

~~4.  **Erweiterter CSV-Export:**
    - Metadaten im Dateinamen, physikalische Header, Bezugssystem-Konstanz.~~ ✓ (in 1.9.2 implementiert)
