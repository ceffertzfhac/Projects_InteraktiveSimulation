...
export function computeK(typeKey, beta) {
  ...
  switch (typeKey) {
    ...
    case 'thick_cylinder': return 0.5 * (1 + b * b);
    case 'thick_sphere': {
      if (b >= 0.999) return 2/3;
      if (b <= 0.001) return 0.4;
      // Formula: k = (2/5) * (1 - beta^5) / (1 - beta^3)
      return 0.4 * (1 - Math.pow(b, 5)) / (1 - Math.pow(b, 3));
    }
    default: return 0.5;
  }
}
...
export function precompute() {
  ...
  for (let t = 0; t <= state.store.simDuration + DT/2; t += DT) {
    ...
    fd.sp_x.push(x_c);
    fd.sp_y.push(state.store.R_m);
    fd.sp_vx.push(v_c);
    fd.sp_vy.push(0);
    fd.sp_vabs.push(Math.abs(v_c));
    fd.sp_ax.push(a_cm);
    fd.sp_ay.push(0);
    fd.sp_aabs.push(Math.abs(a_cm));
  ...
  for (const ck of state.store.compareActive) {
    ...
    const ck_factor = computeK(ck, beta);
