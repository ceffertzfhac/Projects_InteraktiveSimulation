...
- **Diagramm-Steuerung (einheitlich, → BACKLOG I12):** Diagramm-*Inhalt* (Typ-Picker, Mehrfach-Modus, Subjekt-Wahl) sitzt in einer **`.graph-toolbar` direkt am Diagramm** (im Center, neben der Graph-Legende) — Proximity: Steuerung am beeinflussten Objekt, **nicht** in der linken Sidebar. Die *räumliche Anordnung* (Sim/Diagramm nebeneinander↔übereinander) als `layout_toggle`-Button in der **Topbar** — zwei getrennte Belange, nicht vermischen. Typ-Picker: `<select id="graph_select[_n]" class="graph-sel">` mit der **gemeinsamen Graph-Toolbar-Select-Klasse aus `shared`** (die lokale `.graph-sel` wird nach `shared/css/design-system.css` promoted, **nicht** 4× dupliziert; **nicht** das Sidebar-`select-field`, `width:100%` zu breit). Optionen dynamisch aus einer `GRAPH_OPTIONS`-Map in `constants.js`. Mehrfach-Modus (optional, nur bei Vergleich mehrerer Subjekte/Größen): **ein** Kontrakt `name="diagram_mode"` · Widget `.speed-pill` · Werte `1`/`2` (nicht die Atwood-Variante `graph_mode`/`radio-pill`/`single`/`dual`). Erlaubte Opt-outs: statische Sims (3massen/Lorentz) & Werkzeuge (§7) ohne Steuerung; Einzel-Diagramm-Sims ohne Mehrfach-Modus; räumliche Bahnkurve y(x) darf im Typ-Picker stehen. Voll-Rezept: `global_docs/simulation_instruction.md` §3 „Diagramm-Steuerung (Platzierung & Mechanik)".
