...
- **Vektor-Pfeilspitzen (Marker `refX` + Schaft-Kürzung):** Simulations-Vektoren (Ort/Geschwindigkeit/Beschleunigung + ihre x/y-Komponenten) müssen `refX = markerWidth` nutzen, sodaß die Pfeilspitze **exakt auf dem Vektor-Endpunkt** sitzt und den Vektor nicht verlängert. Bei `refX=0` ragt die Spitze um `markerWidth · strokeWidth` über den Endpunkt hinaus — der Vektor endet dann jenseits des Zielpunkts (z. B. außerhalb des Kreisradius beim Ortsvektor, oder die Zerlegungs-Pfeile enden jenseits der Punkt-Koordinaten). Das ist ein bekannter, oft nur teilweise behobener Bug (Lorentz/rolling_bodies nutzen `refX = markerWidth − 1` → noch 1 Einheit·strokeWidth Überschiebung); sauber ist nur `refX = markerWidth` (Spitze = Endpunkt, Polygon liegt komplett hinter dem Endpunkt). **Zusätzlich muß der Schaft um die Marker-Länge (`markerWidth · strokeWidth` px) am Endpunkt gekürzt werden**, sodaß er an der Dreieck-Basis endet (nicht an der Spitze): das Dreieck läuft auf eine Punkt-Spitze zu (0 Breite), der Schaft ist konstant `1 · strokeWidth` breit — nahe der Spitze ist das Dreieck schmaler als der Schaft, und die Schaft-Kanten gucken seitlich aus der Spitze heraus. Kurz man den Schaft um die Marker-Länge in Vektor-Richtung (Hilfe `shortenEnd(x1,y1,x2,y2, markerWidth·strokeWidth)`), überdeckt das deckend gefüllte Dreieck den Schaft vollständig. Marker-Fills pro Vektorfarbe via CSS (`#<id> polygon { fill: var(--c-…) }`), da das Polygon sonst schwarz rendert. **Ausnahme:** Graph-Achsenpfeile (`#graph-arrowhead`) bleiben bewußt auf `refX=0` **ohne** Schaft-Kürzung, weil das Graph-bg-Rect um die Pfeilspitze herum dimensioniert ist (Regel „10 px past arrow tips").
