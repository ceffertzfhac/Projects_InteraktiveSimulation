...
**Issue/Backlog tracking (single source of truth):** `BACKLOG.md` (repo root) is the **one** MoSCoW-prioritized tracker across all sims — bugs (`B#`), tech debt (`T#`), features (`F<sim>#` + repo-wide `I#`), standalone migrations (`M#`), and new-simulation ideas (`N#`). It replaced the per-sim `docs/issues.md`/`docs/FEATURE_BACKLOG.md` (those were retired 2026-07-08 — see `## KONVENTIONEN` at the top of `BACKLOG.md`). **Entry-point rule:** an incoming bug report is always filed as a new `B#` in `BACKLOG.md` `## BUGS`, never only in a per-sim file. Per simulation, only `docs/CHANGELOG.md` (shipped versions) and optional `docs/KNOWN_LIMITATIONS.md` (deliberate local limitations / Won't / scope decisions, with `→ <ID>` links back to `BACKLOG.md`) are kept. Check `BACKLOG.md` before starting work (known issues) and after finishing (record follow-ups). `README.md` (repo overview) and `AGENTS.md` (agent quick-reference) are current and kept in sync with this file, but this file remains canonical — on any conflict, prefer this file and `BACKLOG.md`.
...
- **Graph title z-order:** `<text class/id="graph_title">` must be the **last child among the data elements** inside the graph SVG — after `<g class="graph_grid">`, after all `<polyline>` and `<circle>` data elements. This ensures the title is rendered on top of data lines and the white background rect. Never place it before polylines/circles. **Mit Hover-Overlay (§ „Hover-Werte am Zeit-Diagramm"):** das Hover-Overlay (Cursor/Tooltip) und als ganz letztes das unsichtbare `graph_hit_rect` folgen *nach* dem Titel — kein Konflikt, da der Cursor innerhalb der Plot-Fläche bleibt und den Titelbereich nicht überdeckt (Hit-Rect ist `fill:none`, nur pointer-aktiv).
...
- **Zwei-Diagramm-Anordnung orthogonal zur Sim/Diagramm-Aufteilung** (canonical reference: Kreis-/Spiralbewegung v1.4.0, Kreisbewegung v1.1.0): Bei Sims mit Nebeneinander-/Übereinander-Layout-Umschalter **und** Zweier-Diagramm-Modus liegen die beiden Diagramme **orthogonal zur Sim/Diagramm-Aufteilung** — Nebeneinander-Layout → Diagramme rechts **übereinander** gestapelt; Übereinander-Layout → Diagramme unten **nebeneinander**. Die Mittellinie (Sim/Diagramm-Trenner, Grid-Partition) verschiebt sich nicht, nur die Anordnung *innerhalb* der Diagrammzelle. Grund: gestapelte Teilgraphen in einer breit-flachen Zelle werden sehr flach/unleserlich, nebeneinander in einer hohen Zelle winzig. Technisch in `render.js`: Geometrie-Funktion liefert pro Diagramm `cellW`/`cellH` + `off2`-Versatz; Übereinander-Dual → viewBox-Breite `2·Diagramm+Gap` + X-Versatz, Nebeneinander-Dual → viewBox-Höhe `2·Slot+Gap` + Y-Versatz. Voll-Rezept in `global_docs/simulation_instruction.md`.
...
- **Physik-Logo (FB 8):** Jede Sim zeigt das Logo „Lehr- und Forschungsgebiet Physik“ in der Topbar als `<img src="../shared/img/logo_physik_mint.png" class="topbar-logo" alt="…" title="…">` (41 px, leicht transparent; Klasse in `shared/css/design-system.css`, im Scaffold enthalten). **Header-Aufbau (zweizeilig, für schmale Bildschirme):** `.topbar-title` > `.topbar-title-text` (Zeile 1 `.topbar-title-line` = Titel · `.version` · `.inst`, Zeile 2 `.copyright` = „© 2026 FH Aachen, FB 8 · Alle Rechte vorbehalten", kleiner) + das Logo **rechts daneben über beide Zeilen**. Kein Inline-Style am Copyright — Styling zentral in `design-system.css`. Übersichtsseiten (`AllAnimations/`, `Webpage/index.html`) prominenter als `.header-logo` (62 px) links vor dem Brand-Text. Web-Fassung `shared/img/logo_physik_mint.png` (160 px hoch), Original in `shared/img/src/` — nicht das Original einbinden. `shared/img/*.png` wird von `sync-webpage.sh` mitgespiegelt.
- **Sidebar widths:** Default `grid-template-columns: 280px 1fr 270px` (in `shared/css/design-system.css`); dokumentierte Ausnahme `kreis_spiralbewegung` (`280px 1fr 405px`, breiteres rechtes Panel für den I9-Dualgraph). Eingeklappter Analyse-Panel → `280px 1fr 44px` (transient, s. „Einklappbare Analyse-Sidebar").
...
- **Hover-Werte am Zeit-Diagramm (canonical reference: Zykloide v1.1.0, → BACKLOG I5):** Mouseover über ein Wert-vs.-Zeit-Diagramm zeigt einen Cursor (gestrichelte Führungslinie + hohle Ring-Punkte je Subjekt, unterscheidbar vom gefüllten Wiedergabe-Marker) und ein Tooltip mit den exakten Werten zum gehoverten Zeitpunkt, via bestehendem `interpolateAt(t)` — keine neue Interpolation. Neues `shared/js/hover.js` (`svgLocalPoint`/`attachGraphHover`): CTM wird auf dem **Hit-Rect selbst** aufgerufen, nie auf dem äußeren `<svg>`, damit dieselbe Funktion unverändert bei eigener Graph-SVG, bei einer Graph-Gruppe in einer geteilten Szene-SVG und bei transformierten Dual-Graph-Slots funktioniert. Hit-Rect-Geometrie wird bei jedem Zeichnen aus denselben Lokalen wie `scaleT`/`scaleY` synchronisiert (nie hartkodiert) und lebt als stabiles Geschwister-Element außerhalb der oft per `innerHTML=''` weggeworfenen Grid-Gruppe. Cursor bleibt auf den bereits gezeichneten Kurvenabschnitt geklammert (`t ∈ [0, min(time_range, simulatedTime)]`); bei wachsendem/scrollendem Zeitfenster korrigiert sich ein offener Hover am Ende jeder Zeichenfunktion selbst nach (`if (store.hoverActive) updateGraphHover(...)`). Räumliche/nicht-monotone Bahnkurven-Diagramme sind out of scope (Nearest-Point-Suche nötig, separates Feature). Voll-Rezept in `global_docs/simulation_instruction.md` § "Hover-Werte am Zeit-Diagramm". **Rollout abgeschlossen** (Rollende Körper v2.1.0, Schräger Wurf v1.3.0, Kreis-/Spiralbewegung v1.5.0) — je eine sim-spezifische Anpassung: Rollende Körper schließt Vergleichskörper aus; Schräger Wurf hat 3 Diagramm-Slots als `<g>`s in derselben Szene-SVG; Kreis-/Spiralbewegung braucht wegen `innerHTML=''`-Vollleerung eigene Geschwister-Gruppen für Hit-Rect/Overlay statt verschachtelter Kinder.
- **Einklappbare Bedien-Sidebar links (→ BACKLOG I17, PO-Wunsch 2026-09-26):** The left sidebar is collapsible **as a whole**, mirroring the analysis sidebar but **default expanded**. Every sim includes `<script type="module" src="../shared/js/panel-toggle.js"></script>` before its main script; the module inserts the „Bedienung" header (`.panel-header.controls-header`) and toggles `.controls-collapsed` on `.app-layout`. CSS in `shared/css/design-system.css` („Einklappbare Bedien-Sidebar"): 44-px rail, right width via `--right-w` (default 270px, Kreis-/Spiral 405px); sims that stack the grid on narrow screens override in their media query (Lineal). Recipe: `global_docs/simulation_instruction.md`.
- **Akkordeon-Steuerungs-Sidebar links (canonical reference: Kreis-/Spiralbewegung v1.3.0):** In addition to I17, each `.panel-section` cluster is individually collapsible (accordion). *(Until 2026-09-26 the left sidebar was never collapsed as a whole — superseded by I17.)* `.panel-label` becomes a `<button>` with a large chevron `▾` (rotates `-90°`→`▸` when collapsed) + `aria-expanded`; content hidden via `.panel-section.collapsed > .panel-label ~ * { display: none !important }` — the `!important` is required so JS-toggled `display:block` children don't bleed through when collapsed. `display:none` is safe here (left sidebar = static MathJax labels, typeset at load — no off-screen trick needed, unlike the right analysis panel). **Cluster-Prinzipien vor dem Konvertieren prüfen:** verwandte kleine Cluster zusammenlegen, Single-Control-Sektionen integrieren, Legende direkt nach Visualisierung, ~4–6 Cluster anstreben; default eingeklappt = selten Genutztes. Per sim neu entscheiden which clusters default-collapsed. Full recipe + Gotchas in `global_docs/simulation_instruction.md` § "Akkordeon-Steuerungs-Sidebar". Apply to sims with overlong left sidebars → BACKLOG I8.
...
- **Webpage-Deploy-Sync (→ BACKLOG I11):** `Webpage/sim_*/` ist ein Mirror der kanonischen `Project_*_simulation/` (einzige Transform: Back-Button-href `../AllAnimations/index.html` → `../index.html`). Nach **jeder** Änderung an `Project_*/` oder `shared/` **vor dem Commit** `bash scripts/sync-webpage.sh` laufen lassen und `bash scripts/check-webpage-drift.sh` (exit 0) bestätigen — sonst driftet die öffentliche Pages-Site. Der Deploy-Workflow (`.github/workflows/deploy-pages.yml`) führt den Drift-Check als Guard aus (schlägt bei Drift fehl → kein Deploy) und deployt nur bei Commits, die `Webpage/**`/`scripts/`/den Workflow selbst berühren (reine Docs-Commits deployen nicht). `Webpage/index.html` und `Webpage/Vorschaubilder/` sind Webpage-only (kein Project_-Gegenstück, nicht vom Sync betroffen). **Nicht jede Sim ist öffentlich (→ BACKLOG I15):** beide Skripte führen neben `SIMS` eine Liste `NICHT_OEFFENTLICH`; dort eingetragene Sims bleiben als `Project_*_simulation/` im Repo und auf der internen Übersicht `AllAnimations/`, werden aber **nicht** nach `Webpage/` gespiegelt. Der Drift-Check ist zugleich Publikations-Guard — er schlägt fehl, wenn ein `Webpage/sim_<ausgeschlossen>/` existiert **oder** `Webpage/index.html` darauf verlinkt, und blockiert damit im Workflow den Deploy. Eine ausgeschlossene Sim **niemals** eigenmächtig nach `Webpage/` spiegeln oder in `Webpage/index.html` verlinken — das ist eine PO-Entscheidung. Aktuell ausgeschlossen: keine (`lineal` am 2026-09-18 vom PO freigegeben).
...
- **Diagramm-Steuerung (einheitlich, → BACKLOG I12):** Diagramm-*Inhalt* (Typ-Picker, Mehrfach-Modus, Subjekt-Wahl) sitzt in der **linken Sidebar** in einem `panel-section`-Cluster „Diagramm(e)" — alle Steuerung an einem Ort, **nicht** in einer `.graph-toolbar` am Diagramm (PO-Entscheidung 2026-07-15, kehrt die frühere „Graph-Toolbar-Schule" um). Die *räumliche Anordnung* (Sim/Diagramm nebeneinander↔übereinander) als `layout_toggle`-Button in der **Topbar** — zwei getrennte Belange. Typ-Picker: `<select id="graph_select_1"[/_2] class="select-field">` (Sidebar-Standard, vollbreit) — **nicht** `.graph-sel`; im 2-Diagramm-Modus zwei **unabhängige** Picker (Slot 1 und 2 je eigener Typ, z. B. x(t) und v_y(t) frei mischbar), `#dual_graph_control` blendet den zweiten ein. Optionen dynamisch aus einer `GRAPH_OPTIONS`-Map in `constants.js`. Mehrfach-Modus (optional): **ein** Kontrakt `name="diagram_mode"` · Widget `.speed-pill` · Werte `1`/`2` (Labels „Ein Diagramm"/„Zwei Diagramme") — nicht die Atwood-Variante `graph_mode`/`radio-pill`/`single`/`dual`. Sonderfall Atwood-Energie: das Energie-Balkendiagramm ist ein **Typ im Picker**, nur im Ein-Diagramm-Modus verfügbar (kein eigener `diagram_mode`-Wert `bars`). Erlaubte Opt-outs: statische Sims (3massen/Lorentz) & Werkzeuge (§7) ohne Steuerung; Einzel-Diagramm-Sims ohne Mehrfach-Modus; räumliche Bahnkurve y(x) darf im Typ-Picker stehen und erzwingt den 1-Modus. Voll-Rezept: `global_docs/simulation_instruction.md` §3 „Diagramm-Steuerung (Platzierung & Mechanik)". **Referenz:** kreis_spiralbewegung.
...
- **Vektor-Pfeilspitzen (kanonische Geometrie — eine konsistente Kombination, KEINE Doppelkompensation):** Simulations-Vektoren (Ort/Geschwindigkeit/Beschleunigung + ihre x/y-Komponenten) sollen mit der Pfeilspitze **exakt auf dem Zielpunkt** enden (nicht zu lang, nicht zu kurz) und dürfen keinen Schaft seitlich aus der Spitze zeigen. Das Polygon `points="0 0, 5 1.75, 0 3.5"` hat die **Basis bei local x=0** und die **Spitze bei local x=markerWidth (=5)**; `markerUnits=strokeWidth` skaliert alles mit `strokeWidth`, Marker-Länge = `markerWidth · strokeWidth` px. Zwei Stellschrauben, die **zusammen genau einmal** kompensiert werden müssen:
  ...
  - **Zu kurze Vektoren (`len ≤ by`, → BACKLOG B23):** `shortenEnd` gibt bei Vektorlängen ≤ Marker-Länge **`null`** zurück (mit festem Marker geometrisch unmöglich, Spitze aufs Ziel zu klemmen UND `orient="auto"` zu halten — der Schaft bräuchte negative Länge). Aufrufer **müssen** null abfangen und den Vektor verbergen (`if (!end) { el.style.visibility='hidden'; return }` bzw. bei build-and-return-Helfern eine `display:'none'`-Linie zurückgeben) — kein Pfeil bei zu kurzem Vektor statt fehlerhafter Überschieß-Spitze. Ausnahme: Pfeile mit **fester Länge ≫ Marker-Länge** (Achsen, Legenden-Swatch, Lorentz-Kraft-/Strompfeile via Log-Skalierung+`>0`-Guard) erreichen `len ≤ by` nie und brauchen keinen null-Check.
...
- **Standalone-Prototypen (`AllAnimations/*.html`) — Design-System-Anbindung:** `<link rel="stylesheet" href="../shared/css/design-system.css">` **vor** dem Inline-`<style>` einbinden. Surface-/Text-/Border-Vars (`--bg-color`/`--text-color`/`--control-bg`/`--label-color`/`--border-color` o. Ä.) als Alias auf shared Tokens (`var(--bg)`/`var(--surface)`/`var(--text)`/`var(--text2)`/`var(--border)`) mappen — **zwingend auf `body` (nicht `:root`)**. Grund: CSS-Variablen lösen **eager** am deklarierenden Element; Kinder erben den bereits aufgelösten Wert (W3C css-variables-1, CSSWG #2793). Da `body.dark` die Tokens auf `body` redefiniert, würde ein Remap auf `:root` `var(--bg)` mit dem **Light**-Wert einfrieren → Dark Mode greift nicht (war der R4/R5/R6-Fehler, s. Session 2026-07-08). Auf `body` deklariert, wird mit der `body.dark`-Kaskade korrekt (dark/light) aufgelöst und an die Kinder vererbt — **kein separater `body.dark`-Block** nötig. Konstante Marken-/Akzentfarben (`--fh-blue: var(--fh-mint)`, `--fh-red`, `--fh-green`, Plot-/Vektorfarben) dürfen auf `:root` bleiben, da sie nicht dark-mode-abhängig sind. Markenfarbe `#005eb1` → `var(--fh-mint)` (in CSS) bzw. `#00B1AC` (literales Hex in SVG-`fill`/`stroke`-Attributen — `var()` funktioniert **nicht** als SVG-Attribut). Font-Stacks: `system-ui,…` → `var(--font-ui)`, `monospace` → `var(--font-mono)`. Plot-/Vektor-/Körperfarben als lokale Hex belassen. Fixed Back-Button (`← Übersicht`) + Theme-Toggle als `position:fixed`-Overlay direkt nach `<body>` (stört zentrierte Flex-Layouts nicht). Pro Datei bewerten, welche Elemente sinnvoll sind — nicht jeder Prototyp braucht Back-Button/Toggle/Dark Mode (z. B. rein Diagrammatisches); unpassende weglassen.
- **Dark-Mode-LocalStorage-Key:** Einheitlich `fh_theme` (Unterstrich) auf **allen** Seiten (Übersicht, Standalones), damit der Dark-Mode-Status beim Navigieren Übersicht↔Sim persistiert. Nicht `fh-theme` (Bindestrich) o. Ä.
