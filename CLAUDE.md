...
- **Diagramm-Steuerung (einheitlich, → BACKLOG I12):** Diagramm-*Inhalt* (Typ-Picker, Mehrfach-Modus, Subjekt-Wahl) sitzt in **einem** linken Akkordeon-Cluster „Diagramm(e)" **nach Visualisierung→Legende** (Referenz-Platzierung: Kreis-/Spiralbewegung v1.3.0); die *räumliche Anordnung* (Sim/Diagramm nebeneinander↔übereinander) als `layout_toggle`-Button in der **Topbar** — zwei getrennte Belange, nicht vermischen. Typ-Picker: `<select id="graph_select[_n]" class="select-field">` (Shared-Klasse, **nicht** lokales `graph-sel`), Optionen dynamisch aus einer `GRAPH_OPTIONS`-Map in `constants.js`. Mehrfach-Modus (optional, nur bei Vergleich mehrerer Subjekte/Größen): **ein** Kontrakt `name="diagram_mode"` · Widget `.speed-pill` · Werte `1`/`2` (nicht die Atwood-Variante `graph_mode`/`radio-pill`/`single`/`dual`). Erlaubte Opt-outs: statische Sims (3massen/Lorentz) & Werkzeuge (§7) ohne Steuerung; Einzel-Diagramm-Sims ohne Mehrfach-Modus; Plural-Label „Diagramme" nur bei Dual-fähigen; räumliche Bahnkurve y(x) darf im Typ-Picker stehen. Voll-Rezept: `global_docs/simulation_instruction.md` §3 „Diagramm-Steuerung (Platzierung & Mechanik)".
